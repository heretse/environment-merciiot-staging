...
if supports_scc; then
  ...
  if command -v oc >/dev/null 2>&1 ; then
    oc adm policy add-scc-to-group merc-layout-svc-scc system:serviceaccounts:$namespace
  else
...
if supports_psp; then
  ...
  sed 's/{{ NAMESPACE }}/'$namespace'/g' merc-layout-svc-rb.yaml > $namespace-merc-layout-svc-rb.yaml
  ...
  kubectl create -f $namespace-merc-layout-svc-rb.yaml -n $namespace
