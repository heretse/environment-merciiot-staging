...
[am-svc] is an account management service and provides API about user and admin control.
...
This chart bootstraps a [am-svc] deployment on a [Kubernetes](http://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
## Chart Details
  ...
  - Installs one `Deployment` running am-svc image
  - Installs a `Service` and optionally an `Ingress` to route traffic to am-svc


## Prerequisites

### am-svc Docker image requirements

am-svc Docker images based on Universal Base Images (UBI) are publicly available from [Docker hub](https://cloud.docker.com/u/merciiot/repository/docker/merciiot/am-svc) and used by chart as default. Ensure your Kubernetes environment has set the image enforcement policy appropriately to allow access to those repositories. See [Enforcing container image security](https://www.ibm.com/support/knowledgecenter/en/SSBS6K_3.2.0/manage_images/image_security.html) for more information.
...
Download the following scripts located at [/pak_extensions/pre-install](https://github.com/heretse/environment-merciiot-staging/tree/master/am-svc/pak_extensions/pre-install) directory.
...
Download the following scripts located at [/pak_extensions/post-delete](https://github.com/heretse/environment-merciiot-staging/tree/master/am-svc/pak_extensions/post-delete) directory.
...
See [RELEASENOTES.md](https://github.com/heretse/environment-merciiot-staging/tree/master/am-svc/RELEASENOTES.md)
...
*    `helm install --name my-release --set resources.constraints.enabled=true --set autoscaling.enabled=true --set autoscaling.minReplicas=1 merciiot/am-svc --tls`
