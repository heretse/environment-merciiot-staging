...
if supports_scc; then
  ...
  if command -v oc >/dev/null 2>&1 ; then
    oc adm policy remove-scc-from-group merc-mongodb-scc system:serviceaccounts:$namespace
  else
...
if supports_psp; then
  ...
  sed 's/{{ NAMESPACE }}/'$namespace'/g' ../../pre-install/namespaceAdministration/merc-mongodb-rb.yaml > $namespace-merc-mongodb-rb.yaml

  # Delete the role binding for all service accounts in the current namespace
  kubectl delete -f $namespace-merc-mongodb-rb.yaml -n $namespace
