...
if supports_scc; then
  ...
  if command -v oc >/dev/null 2>&1 ; then
    oc adm policy remove-scc-from-group merc-device-svc-scc system:serviceaccounts:$namespace
  else
...
if supports_psp; then
  ...
  sed 's/{{ NAMESPACE }}/'$namespace'/g' ../../pre-install/namespaceAdministration/merc-device-svc-rb.yaml > $namespace-merc-device-svc-rb.yaml

  # Delete the role binding for all service accounts in the current namespace
  kubectl delete -f $namespace-merc-device-svc-rb.yaml -n $namespace

  # Clean up - delete the temporary yaml file.
  rm $namespace-merc-device-svc-rb.yaml
fi