...
if supports_scc; then
  ...
  if command -v oc >/dev/null 2>&1 ; then
    oc adm policy add-scc-to-group merc-device-svc-scc system:serviceaccounts:$namespace
  else
...
if supports_psp; then
  ...
  sed 's/{{ NAMESPACE }}/'$namespace'/g' merc-device-svc-rb.yaml > $namespace-merc-device-svc-rb.yaml
  ...
  kubectl create -f $namespace-merc-device-svc-rb.yaml -n $namespace
