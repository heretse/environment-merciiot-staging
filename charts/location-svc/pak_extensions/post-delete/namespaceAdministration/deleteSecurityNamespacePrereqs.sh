...
if supports_scc; then
  ...
  if command -v oc >/dev/null 2>&1 ; then
    oc adm policy remove-scc-from-group merc-location-svc-scc system:serviceaccounts:$namespace
  else
...
if supports_psp; then
  ...
  sed 's/{{ NAMESPACE }}/'$namespace'/g' ../../pre-install/namespaceAdministration/merc-location-svc-rb.yaml > $namespace-merc-location-svc-rb.yaml

  # Delete the role binding for all service accounts in the current namespace
  kubectl delete -f $namespace-merc-location-svc-rb.yaml -n $namespace

  # Clean up - delete the temporary yaml file.
  rm $namespace-merc-location-svc-rb.yaml
fi