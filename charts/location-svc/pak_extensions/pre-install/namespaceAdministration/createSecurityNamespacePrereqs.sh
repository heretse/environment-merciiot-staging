...
if supports_scc; then
  ...
  if command -v oc >/dev/null 2>&1 ; then
    oc adm policy add-scc-to-group merc-location-svc-scc system:serviceaccounts:$namespace
  else
...
if supports_psp; then
  ...
  sed 's/{{ NAMESPACE }}/'$namespace'/g' merc-location-svc-rb.yaml > $namespace-merc-location-svc-rb.yaml
  ...
  kubectl create -f $namespace-merc-location-svc-rb.yaml -n $namespace
