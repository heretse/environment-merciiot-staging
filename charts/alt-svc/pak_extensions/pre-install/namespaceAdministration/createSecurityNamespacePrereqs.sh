...
if supports_scc; then
  ...
  if command -v oc >/dev/null 2>&1 ; then
    oc adm policy add-scc-to-group merc-alt-svc-scc system:serviceaccounts:$namespace
  else
...
if supports_psp; then
  ...
  sed 's/{{ NAMESPACE }}/'$namespace'/g' merc-alt-svc-rb.yaml > $namespace-merc-alt-svc-rb.yaml
  ...
  kubectl create -f $namespace-merc-alt-svc-rb.yaml -n $namespace
