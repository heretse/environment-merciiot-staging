...
if supports_scc; then
  ...
  if command -v oc >/dev/null 2>&1 ; then
    oc adm policy add-scc-to-group merc-report-svc-scc system:serviceaccounts:$namespace
  else
...
if supports_psp; then
  ...
  sed 's/{{ NAMESPACE }}/'$namespace'/g' merc-report-svc-rb.yaml > $namespace-merc-report-svc-rb.yaml
  ...
  kubectl create -f $namespace-merc-report-svc-rb.yaml -n $namespace
