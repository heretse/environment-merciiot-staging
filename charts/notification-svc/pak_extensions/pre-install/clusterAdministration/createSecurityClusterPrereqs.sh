...
if supports_scc; then 
  ...
  kubectl apply -f merc-notification-svc-scc.yaml --validate=false
fi

if supports_psp; then 
  # Create the PodSecurityPolicy and ClusterRole for all releases of this chart.
  echo "Creating the PodSecurityPolicy..."
  kubectl apply -f merc-notification-svc-psp.yaml
  kubectl apply -f merc-notification-svc-cr.yaml
fi