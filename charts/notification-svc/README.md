# notification-svc Helm Chart


[notification-svc] is a notification service and provides API to access notify objects.
...
This chart bootstraps a [notification-svc] deployment on a [Kubernetes](http://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
## Accessing notification-svc

From a browser, use http://*external-ip*:*nodeport* to access the application.

## Chart Details

  - Installs one `Deployment` running notification-svc image
  - Installs a `Service` and optionally an `Ingress` to route traffic to notification-svc


## Prerequisites

### notification-svc Docker image requirements

notification-svc Docker images based on Universal Base Images (UBI) are publicly available from [Docker hub](https://cloud.docker.com/u/merciiot/repository/docker/merciiot/notification-svc) and used by chart as default. Ensure your Kubernetes environment has set the image enforcement policy appropriately to allow access to those repositories. See [Enforcing container image security](https://www.ibm.com/support/knowledgecenter/en/SSBS6K_3.2.0/manage_images/image_security.html) for more information.
...
metadata:
  name: merc-notification-svc-psp
spec:
...
kind: ClusterRole
metadata:
  name: merc-notification-svc-clusterrole
rules:
- apiGroups:
  - extensions
  resourceNames:
  - merc-notification-svc-psp
  resources:
  - podsecuritypolicies
  verbs:
  - use
...
Download the following scripts located at [/pak_extensions/pre-install](https://github.com/heretse/environment-merciiot-staging/tree/master/notification-svc/pak_extensions/pre-install) directory.
...
Download the following scripts located at [/pak_extensions/post-delete](https://github.com/heretse/environment-merciiot-staging/tree/master/notification-svc/pak_extensions/post-delete) directory.
...
* From the user interface, you can copy and paste the following snippets to enable the custom `SecurityContextConstraints`
  * Custom `SecurityContextConstraints` definition:
  ...
  metadata:
    annotations:
    name: merc-notification-svc-scc
...
See [RELEASENOTES.md](https://github.com/heretse/environment-merciiot-staging/tree/master/notification-svc/RELEASENOTES.md)
...
*    `helm install --name my-release --set resources.constraints.enabled=true --set autoscaling.enabled=true --set autoscaling.minReplicas=1 merciiot/notification-svc --tls`
