...
[mysqldb] is a relational database management system and store structure data for account info and device info.


## Introduction

This chart bootstraps a [mysqldb] deployment on a [Kubernetes](http://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
## Chart Details
  ...
  - Installs one `Deployment` running mysqldb image
  - Installs a `Service` and optionally an `Ingress` to route traffic to mysqldb


## Prerequisites

### mysqldb Docker image requirements

mysqldb Docker images based on Universal Base Images (UBI) are publicly available from [Docker hub](https://cloud.docker.com/u/merciiot/repository/docker/merciiot/mysqldb-merc) and used by chart as default. Ensure your Kubernetes environment has set the image enforcement policy appropriately to allow access to those repositories. See [Enforcing container image security](https://www.ibm.com/support/knowledgecenter/en/SSBS6K_3.2.0/manage_images/image_security.html) for more information.
...
Download the following scripts located at [/pak_extensions/pre-install](https://github.com/heretse/environment-merciiot-staging/tree/master/mysqldb/pak_extensions/pre-install) directory.
...
Download the following scripts located at [/pak_extensions/post-delete](https://github.com/heretse/environment-merciiot-staging/tree/master/mysqldb/pak_extensions/post-delete) directory.
...
See [RELEASENOTES.md](https://github.com/heretse/environment-merciiot-staging/tree/master/mysqldb/RELEASENOTES.md)
...
*    `helm install --name my-release --set resources.constraints.enabled=true --set autoscaling.enabled=true --set autoscaling.minReplicas=1 merciiot/mysqldb-merc --tls`
