...
if supports_scc; then
  ...
  if command -v oc >/dev/null 2>&1 ; then
    oc adm policy add-scc-to-group merc-mysqldb-scc system:serviceaccounts:$namespace
  else
...
if supports_psp; then
  ...
  sed 's/{{ NAMESPACE }}/'$namespace'/g' merc-mysqldb-rb.yaml > $namespace-merc-mysqldb-rb.yaml
  ...
  kubectl create -f $namespace-merc-mysqldb-rb.yaml -n $namespace
